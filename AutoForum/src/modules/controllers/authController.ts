...
const registration = function (request: http.IncomingMessage, response: http.ServerResponse, username: string): void {
...
const login = (request: http.IncomingMessage, response: http.ServerResponse, username: string): void => {
  getPostData(request).then(
    body => {
      const { username, password } = JSON.parse(body)
...
const logout = (request: http.IncomingMessage, response: http.ServerResponse, username: string): void => {
  getPostData(request).then(
    body => {
